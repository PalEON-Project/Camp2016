...
- We use Bayes theorem to find  $g(\theta|y)$, making use of $f(y|\theta)$, by using a version of Bayes theorem for random variables rather than events:
$$
g(\theta|Y=y)=\frac{f(y|\theta)g(\theta)}{g(y)}\propto f(y|\theta)g(\theta)
$$
- We use the likelihood, $f(y|\theta)$ (pdf or pmf of the data), to update the prior.
...
<center>![fig 1](plots/betaDist.png)</center>

# Revisiting our example

Recall we have the following: $g(\theta|y)\propto\theta^{y}(1-\theta)^{n-y}$

What is the distribution of  $\theta$?

Suppose  $y=35$ and $n=100$.
...
To complete the Bayesian model, we need prior distributions for the parameters
 and for the initial state, $g(\mu, \rho, \sigma^{2},\tau^{2},\theta_{0})$.
...
Apart from our posterior correlation estimate, the other quantities we calculated represent the marginal posteriors:  $g(\theta_{1}|y)$
 and  $g(\theta_{2}|y)$. In this context we can just ignore the samples of the parameters we are
 not interested in.
 We're generally not interested in conditional posteriors such as 
 $g(\theta_{1}|\theta_{2},y)$, but if we were, we could have just fixed 
