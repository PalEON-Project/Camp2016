...
6. Look at the MCMC output and check how well the MCMC is performing. What length of burn-in do you think is needed. How well does the MCMC mix?  Can the variance components be estimated with the flat priors given the data available or is there a lot of uncertainty about the variance components? Are there any variance components that seem to trade off with each other? If so, these are components that the model has trouble identifying given the data available.
